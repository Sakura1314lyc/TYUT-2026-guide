...
- 七类折叠分组、带分类进度并可在浏览器中自动保存的 56 项行李清单
- 首页新生反诈与身份、付款安全提醒
- 8 个章节、67 个栏目组成的完整新生攻略库
- 收录当前公开文档中的 1208 条文字内容和 50 张资料图
- 原文嵌套子页面、暂未填写的目录栏目均保留
- 章节总览、全文搜索、章节筛选和单篇按需展开
- 对日期、金额、名额和政策类内容增加时效核验提醒
- 报到路线和常见问题
- 手机、平板和桌面端响应式布局
- 新生指南宣传海报及网站二维码
...
├─ guide-data.js             # 完整攻略目录、正文和资料图数据
├─ scripts/
│  └─ sync-tencent-guide.mjs # 重新提取公开攻略文档的同步脚本
├─ assets/
│  ├─ *-plan.jpg             # 三校区平面图
│  ├─ campus-map-qr-blue.jpg # 手机校园导览二维码
│  ├─ guide-doc/             # 攻略参考文档中的资料图
│  └─ photos/                # 校园及设施实景照片
└─ poster/
   ├─ poster.html            # 海报页面
   ├─ poster.css             # 海报样式
...
- 校区、设施、坐标和实景图：编辑 `app.js`
- 攻略目录、正文和文档资料图：编辑 `guide-data.js` 与 `assets/guide-doc/`
...
攻略库按参考文档的现有目录、正文、嵌套子页面和资料图进行整理，删除了装饰性 Emoji，但不等同于学校正式通知。校园地图、实景图片和学校相关标识来源于太原理工大学官方公开页面，仅用于新生便民展示。若学校政策、设施、道路或管理安排发生变化，请以录取通知书、学校及学院最新通知和现场标识为准。
